...
export async function mockAPIEndpoint (port) {
  ...
  app.get('/auth/accounts/:address', respond(({address})=>{
    return {
      "height": MockChainState.block.height,
      "result": {
        "type": "cosmos-sdk/Account",
        "value": {
          "address": address,
          "coins": [
            { "denom": "uscrt", "amount": String(MockChainState.state.balances[address]||0) }
          ],
  ...
  app.post('/txs', (_, {tx:{msg}})=>{
    const txhash = randomHex(16).toUpperCase()
    const mockTx = (logs) => {
      MockChainState.txs[txhash] = {
        txhash,
        raw_log: "",
        logs
      }
    ...
    const mockHandlers = {
      'cosmos-sdk/MsgSend' () {
        ...
        for (const {denom, amount: x} of amount) {
          if (denom === 'uscrt') {
            MockChainState.balances[from_address] -= BigInt(x)
            MockChainState.balances[to_address]   += BigInt(x)
          }
        }
        mockTx()
      },
  ...
  app.get('/txs/:txhash', (req, res, next)=>{
    const response = MockChainState.txs[req.params.txhash]
