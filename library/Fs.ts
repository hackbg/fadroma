...
export const Dir: {
  ...
  return Fn.Name(`Dir(${path})`, makeDirectory, props) as Dir.Entry<D>;
...
export function Zip <D extends Dir> (name: string, ...entries: Dir.Entry<D>[]) {
  return Fn.Name(`Zip(${entries.length})`, writeZipFile, { entries })
...
export const Project = (name: string, ...ops: Dir.Entry[]) =>
  Fn.Name(name, Dir(name, Readme(name, `Generated by @hackbg/fadroma.`), ...ops));
