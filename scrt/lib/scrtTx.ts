import { Core, Address, CosmWasm, Tendermint, MsgStoreCode, MsgInstantiateContract, MsgExecuteContract } from '../deps.ts'
...
const encryptBatch = (): Promise<any[]> => {
  const messages: any[] = []
...
async function encryptUpload (init: any): Promise<any> {
  throw new Error('not implemented')
}

async function encryptInit (init: any): Promise<any> {
  return {
...
async function encryptExec (exec: any): Promise<any> {
  return {
...
async function submitBatch ({ memo = "" }: { memo: string }): Promise<BatchResult[]> {
...
function composeUnsignedTx (encryptedMessages: any[], memo?: string): any {
