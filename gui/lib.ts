...
export const append  = (el, ...els) => { els.forEach(e=>el.appendChild(e)); return el; }
export const on = (x: EventTarget, ev, cb) => { x?.addEventListener(ev, cb); return cb; }

export function Icon (name) {
  return ['svg.icon', [`use[href=icons.svg#icon-${name}]`]];
}
