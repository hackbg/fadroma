...
export const fetchEpoch = async ({ fetchAbciQuery }: Deps, height?: Height) => {
  if (height !== undefined) {
    const binary = await fetchAbciQuery(`/shell/epoch_at_height/${height}`)
    return binary[0] ? decode(u64, binary.slice(1)) : null
  }
  return decode(u64, await fetchAbciQuery("/shell/epoch"))
}
...
export const fetchEpochFirstBlock = async ({ fetchAbciQuery }: Deps) =>
  Number(decode(u64, await fetchAbciQuery('/shell/first_block_height_of_current_epoch')))
