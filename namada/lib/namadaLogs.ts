...
import { Tendermint, bold } from '../deps.ts'
export class NamadaLogger extends Tendermint.Logger {
  warnNoDecoder = () =>
    this.warn("Decoder binary not provided; trying to decode Namada objects will fail.")
  printValidator = (validator: Validator) => this
  ...
  printTx = (tx: Partial<Transaction> = {}) => this
    .log('-', bold(`${tx.data?.txType} transaction:`))
