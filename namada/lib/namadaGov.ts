...
export const fetchGovernanceParameters = async ({ abciQuery, decoder }: ConnectionBase) =>
  decoder.gov_parameters(await abciQuery(`/vp/governance/parameters`))
export const fetchProposalCount = async ({ abciQuery }: ConnectionBase) =>
  decode(u64, await abciQuery(`/shell/value/#${GOV_INTERNAL_ADDRESS}/counter`)) as bigint
export const fetchProposalInfo = async (
  { abciQuery, decoder }: ConnectionBase, id: number|bigint
