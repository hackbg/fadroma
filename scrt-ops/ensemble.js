...
import {
  bold, resolve, relative, existsSync, taskmaster,
...
export default class ContractEnsemble {
  ...
  constructor(options = {}) {
    const { network, agent, builder = this.builder } = options;
  ...
  async build(options = {}) {
    ...
    return parallel ? buildParallel() : buildSeries();

    async function buildParallel() {
      const binaries = {};
      await task.parallel(`build ${myName}`,
        ...Object.entries(contracts).map(([name, { crate }]) => task(`build ${name}`, async (report) => {
  ...
  async upload(options = {}) {
    ...
    for (const contract of Object.keys(this.contracts)) {
      await task(`upload ${contract}`, async (report) => {
        const receipt = await builder.uploadCached(binaries[contract]);
  ...
  async deploy(options = {}) {
    ...
    return task('build, upload, and initialize contracts', async () => {
      const binaries = await this.build({ task, builder });
      const receipts = await this.upload({
        task, network, builder, binaries,
      });
      const contracts = await this.initialize({
        task, network, receipts, agent,
      });
      return contracts;
    });
  }
