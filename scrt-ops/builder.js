...
export default class SecretNetworkBuilder {
  ...
  constructor(options = {}) {
    const { docker: _docker } = options;
    let { network, agent } = options;
    if (_docker) this.docker = _docker;
    if (!network && agent) {
   ...
   */
  getBuildEnv = () => ['CARGO_NET_GIT_FETCH_WITH_CLI=true',
  ...
  getReceiptPath = (path) => resolve(this.network.receipts, `${basename(path)}.upload.json`)
