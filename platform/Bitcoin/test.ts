...
export function SendFromWallet (amount: string|number|bigint, to?: string|number) {
  const id = (typeof to === 'number')?`NPK#${to}`:to;
  return Fn.Name(`Wallet sends ${amount} to ${id}`, async (context: Btc & { identity, txid, tx }) => {
  ...
  function createTestUser ({ P2WPKH, warn = console.warn, info = console.info }: Btc & {
