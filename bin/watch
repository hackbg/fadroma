#!/usr/bin/env -S deno run -I --allow-env --allow-run
import { stdin, stdout } from 'node:process';
import testSuite from '../lib/index.test.ts';
import { Ansi, Main, Exec, when } from '../lib/index.ts';
import { tuiContext, runInput, runOutput, draw, at } from '../lib/context/tui.ts';
...
export default Main(import.meta, main);

async function main (...args) {
  await inRawTTY(stdin, start, ...args);
});

async function inRawTTY (stdin, cb, ...args) {
...
const status = (state: State) => {
  ...
  return at(0, 0,
    ...
    at(Math.max(0, (state.width - size.length - 1) || 0), 0,
      styled(size), '\n '),
    at(0, state.height - 1, Ansi.bg255(240)(' Filter: '), ' \x1b[1m'+state.filter)
  )(state)
};

const output = draw(title, tests, status);

const styled = (x: String) => '\x1b[1m'+Ansi.fg255(16)(Ansi.bg255(208)(x));
  ...
  function collectSteps (ids, steps: Test.Step<unknown> = []) {
    for (const index in steps) {
      ...
      output.push([
        ...
        Ansi.gray(subids.length, step.name.padEnd(14))+' ',
        Ansi.gray(10, 'ready')
      ].filter(Boolean).join(''));
