...
use fadroma::{
    scrt::{
        OwnedDeps, Deps, DepsMut, Env, MessageInfo,
        StdResult, StdError, Binary, Response, Addr,
        Coin, Uint128, from_binary, to_binary, CosmosMsg,
        WasmMsg, Api
    },
    scrt_crypto::sha_256,
...
use crate::{
    ...
    batch,
    state::{Account, Config, get_admin},
    msg::{
        InitMsg, HandleMsg, QueryMsg, InitialBalance,
        InitConfig, HandleAnswer, QueryAnswer, ResponseStatus,
        ContractStatusLevel
    }
};
...
fn init(
    ...
    crate::init(deps, env, info, msg, DefaultSnip20Impl)
}
...
fn handle(
    ...
    crate::handle(deps, env, info, msg, DefaultSnip20Impl)
}
...
fn query(
    ...
    crate::query(deps, env, msg, DefaultSnip20Impl)
}
...
fn test_init_sanity() {
    ...
    assert_eq!(init_result.unwrap(), Response::new()
    ...
    assert_eq!(Config::get_total_supply(storage).unwrap(), Uint128::new(5000));
    assert_eq!(Config::get_contract_status(storage).unwrap(), ContractStatusLevel::NormalRun);
    assert_eq!(constants.name, "sec-sec".to_string());
    assert_eq!(get_admin(deps.as_ref()).unwrap(), Addr::unchecked("admin".to_string()));
...
fn test_handle_send() {
    ...
    let handle_result = handle(deps.as_mut(), mock_env(), mock_info("contract", &[]), handle_msg);
    let result = handle_result.unwrap();
    assert!(ensure_success(result));
...
fn test_handle_register_receive() {
    ...
    let hash = account.get_receiver_hash(deps.as_ref().storage)
        .unwrap()
        .unwrap();
...
fn test_handle_transfer_from() {
    ...
    let handle_result = handle(deps.as_mut(), mock_env(), mock_info("alice", &[]), handle_msg);
    let error = extract_error_msg(handle_result);
...
fn test_handle_send_from() {
    ...
    let messages: Vec<CosmosMsg> = handle_result.unwrap().messages.into_iter().map(|x| x.msg).collect();
    ...
    let total_supply = Config::get_total_supply(deps.as_ref().storage).unwrap().u128();
...
fn test_handle_burn_from() {
    ...
    let handle_result = handle(deps_for_failure.as_mut(), mock_env(), mock_info("alice", &[]), handle_msg);
    let error = extract_error_msg(handle_result);
...
fn test_handle_decrease_allowance() {
    ...
    let alice_canonical = deps
        .api
        .addr_canonicalize("alice")
        .unwrap();

    let allowance = bob.get_allowance(deps.as_ref().storage, &alice_canonical).unwrap();
    assert_eq!(
        allowance,
...
fn test_handle_change_admin() {
    ...
    let handle_result = handle(deps.as_mut(), mock_env(), mock_info("admin", &[]), handle_msg);
    assert!(
        handle_result.is_ok(),
        "handle() failed: {}",
        handle_result.err().unwrap()
...
fn test_handle_redeem() {
    ...
    let handle_result = handle(deps_for_failure.as_mut(), mock_env(), mock_info("butler", &[]), handle_msg);
    let error = extract_error_msg(handle_result);
    ...
    let handle_result = handle(deps_no_reserve.as_mut(), mock_env(), mock_info("butler", &[]), handle_msg);
    let error = extract_error_msg(handle_result);
    ...
    let handle_result = handle(deps.as_mut(), mock_env(), mock_info("butler", &[]), handle_msg);
    assert!(
        handle_result.is_ok(),
        "handle() failed: {}",
        handle_result.err().unwrap()
...
fn test_handle_burn() {
    ...
    let handle_result = handle(deps_for_failure.as_mut(), mock_env(), mock_info("lebron", &[]), handle_msg);
    let error = extract_error_msg(handle_result);
    assert!(error.contains("Burn functionality is not enabled for this token."));

    let supply = Config::get_total_supply(deps.as_ref().storage).unwrap().u128();
    ...
    let handle_result = handle(deps.as_mut(), mock_env(), mock_info("lebron", &[]), handle_msg);
    assert!(
        handle_result.is_ok(),
        "Pause handle failed: {}",
        handle_result.err().unwrap()
    ...
    let new_supply = Config::get_total_supply(deps.as_ref().storage).unwrap().u128();
...
fn test_handle_mint() {
    ...
    let handle_result = handle(deps_for_failure.as_mut(), mock_env(), mock_info("admin", &[]), handle_msg);
    let error = extract_error_msg(handle_result);
...
fn test_handle_admin_commands() {
    ...
    let handle_result = handle(deps.as_mut(), mock_env(), mock_info("not_admin", &[]), pause_msg);
    let error = extract_error_msg(handle_result);
    ...
    let handle_result = handle(deps.as_mut(), mock_env(), mock_info("not_admin", &[]), mint_msg);
    let error = extract_error_msg(handle_result);
    assert!(error.contains(&admin_err.clone()));
    ...
    let handle_result = handle(deps.as_mut(), mock_env(), mock_info("not_admin", &[]), change_admin_msg);
    let error = extract_error_msg(handle_result);
    assert!(error.contains(&admin_err.clone()));
}
...
fn test_handle_pause_with_withdrawals() {
    ...
    let handle_result = handle(deps.as_mut(), mock_env(), mock_info("admin", &[]), pause_msg);
    assert!(
        handle_result.is_ok(),
        "Pause handle failed: {}",
        handle_result.err().unwrap()
    ...
    let handle_result = handle(deps.as_mut(), mock_env(), mock_info("lebron", &[]), withdraw_msg);
    assert!(
        handle_result.is_ok(),
        "Withdraw failed: {}",
        handle_result.err().unwrap()
...
fn test_authenticated_queries() {
    ...
    let handle_response = handle(deps.as_mut(), mock_env(), mock_info("giannis", &[]), create_vk_msg).unwrap();
    let vk = match from_binary(&handle_response.data.unwrap()).unwrap() {
        HandleAnswer::CreateViewingKey { key } => key,
        _ => panic!("Unexpected result from handle"),
    };
...
fn test_query_allowance() {
    ...
    let handle_result = handle(deps.as_mut(), mock_env(), mock_info("giannis", &[]), handle_msg);
    assert!(
        handle_result.is_ok(),
        "handle() failed: {}",
        handle_result.err().unwrap()
