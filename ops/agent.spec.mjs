...
describe('ScrtJSAgent', function () {
  ...
  it('can be created from a keypair', async function () {
    const keypairAgent = await ScrtJSAgent.create({ keyPair: keypair, network: context.network });
  ...
  it('can be created from a signing pen', async function () {
    const penAgent = await ScrtJSAgent.create({ pen: context.pen, network: context.network });
  ...
  it('can have a name', async function () {
    const namedAgent = await ScrtJSAgent.create({ name: 'TEST', mnemonic, network: context.network });
  ...
  it('can send SCRT to many addresses in one go', async function () {
    this.timeout(120000);
    const agent1 = await ScrtJSAgent.create({
      mnemonic1,
      network: context.network,
    });
    const agent2 = await ScrtJSAgent.create({
      mnemonic2,
      network: context.network,
    });
